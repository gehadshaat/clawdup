...
6. **Link natively:** after the run (unless `NATIVE_STACKS=false`), `linkStackPRs` links the series' open PRs into a **native GitHub stack** (public preview) through the official `gh stack` extension: `gh stack link` chains the already-created PRs bottom-up, and a resumed run extends the stack the bottom PRs already sit in by passing its stack number first. Read-only stack-membership lookups still use the Stacks REST API via `gh api` (the extension has no non-interactive membership query). The extension is a hard requirement of stack runs — `runTaskStack` checks for it up front, offers to install it on an interactive terminal (`gh extension install github/gh-stack`), and aborts otherwise. Linking itself stays best-effort: unavailable stacked PRs (404) or any failure is reported in the summary comment and the chained PRs keep working as before — where GitHub only retargets a dependent PR when the merged head branch is deleted.
