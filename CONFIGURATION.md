...
| `NATIVE_STACKS` | `true` | After a `--stack` run, link the series' open PRs into a **native GitHub stack** via the Stacks REST API (public preview), so merging a lower PR automatically rebases and retargets the PRs above it, and reviewers get GitHub's stack UI. Resumed runs extend the existing stack. Best-effort: where the API is unavailable (404 — the preview isn't enabled for the repository yet) the chained PRs behave as before, and the run summary says so. Set to `false` to disable. |
