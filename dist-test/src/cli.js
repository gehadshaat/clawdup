...
function printRecommendedStatuses() {
    console.log(`
...
async function initProject() {
    ...
    if (existsSync(envDest)) {
        ...
        writeFileSync(envDest, `# ClickUp Task Automation - Environment Variables
...
`);
    ...
    if (existsSync(configDest)) {
        ...
        writeFileSync(configDest, `// clawup.config.mjs
...
async function runChecks({ validateStatuses, getListInfo, getTask, detectGitHubRepo, parentTaskId, }) {
    ...
    try {
        ...
        console.log(`  Statuses: ${listInfo.statuses.map((s) => s.status).join(", ")}`);
    }
    catch (err) {
        console.error(`  ClickUp API: FAILED - ${err.message}`);
        allGood = false;
    }
    // Validate statuses
    try {
        ...
        console.error(`  Status validation: FAILED - ${err.message}`);
        allGood = false;
    }
    // Check GitHub
    try {
    ...
    try {
        ...
        console.error('  Claude Code: FAILED - "claude" command not found or not working');
        allGood = false;
    }
    // Check gh CLI
    try {
    ...
    if (existsSync(claudeMd)) {
        ...
        console.log("  CLAUDE.md: not found (optional — add one for better task context)");
    }
