 ...
 */
export async function createTaskBranch(taskId, slug) {
    ...
    for (let attempt = 0; attempt <= delays.length; attempt++) {
        try {
            ...
            if (attempt < delays.length) {
                ...
                log("warn", `Push failed, retrying in ${delay / 1000}s: ${err.message}`);
 ...
 * Returns the PR URL.
 */
export async function createPullRequest({ title, body, branchName, baseBranch, draft, }) {
 ...
 */
export async function createEmptyCommit(message) {
 ...
 */
export async function markPRReady(prUrl) {
 ...
 */
export async function closePullRequest(prUrl) {
 ...
 */
export async function updatePullRequest(prUrl, { title, body }) {
...
export async function findExistingPR(branchName) {
    try {
        const prUrl = await gh("pr", "view", branchName, "--json", "url", "--jq", ".url");
        return prUrl || null;
    }
    catch {
        return null;
 ...
 */
export async function findBranchForTask(taskId) {
    // Check local branches
    const localResult = await git("branch", "--list", `${BRANCH_PREFIX}/CU-${taskId}-*`);
    if (localResult) {
    ...
    const remoteResult = await git("branch", "-r", "--list", `origin/${BRANCH_PREFIX}/CU-${taskId}-*`);
    if (remoteResult) {
 ...
 */
export async function checkoutExistingBranch(branchName) {
 ...
 */
export async function branchHasCommitsAheadOfBase() {
 ...
 */
export async function branchHasBeenPushed(branchName) {
    try {
 ...
 */
export async function mergePullRequest(prUrl) {
 ...
 */
export async function getPRMergeability(prUrl) {
    try {
        const result = await gh("pr", "view", prUrl, "--json", "mergeable", "--jq", ".mergeable");
 ...
 */
export async function getPRState(prUrl) {
    const state = await gh("pr", "view", prUrl, "--json", "state", "--jq", ".state");
 ...
 */
export async function getPRReviewDecision(prUrl) {
    try {
        const result = await gh("pr", "view", prUrl, "--json", "reviewDecision", "--jq", ".reviewDecision");
 ...
 */
export async function getPRReviewComments(prUrl) {
    try {
        const result = await gh("pr", "view", prUrl, "--json", "reviews", "--jq", '[.reviews[] | select(.body != "") | {author: .author.login, body: .body, createdAt: .submittedAt}]');
        if (!result || result === "[]")
            return [];
 ...
 */
export async function getPRInlineComments(prUrl) {
    try {
        ...
        const result = await gh("api", `repos/${repo}/pulls/${prNumber}/comments`, "--jq", '[.[] | {author: .user.login, body: .body, path: .path, line: .line, createdAt: .created_at}]');
        if (!result || result === "[]")
            return [];
        return JSON.parse(result);
    }
