...
async function request(method, path, body = null) {
    ...
    if (!res.ok) {
        throw new Error(`ClickUp API error ${res.status} ${method} ${path}: ${text}`);
    }
 ...
 */
async function getEffectiveListId() {
    if (CLICKUP_LIST_ID)
        return CLICKUP_LIST_ID;
    if (resolvedListId)
        return resolvedListId;
    const parent = await request("GET", `/task/${CLICKUP_PARENT_TASK_ID}`);
    if (!parent.list?.id) {
        throw new Error("Could not determine list ID from parent task. Ensure the parent task exists.");
    }
 ...
 */
async function getListTasksByStatus(status) {
    ...
    const data = await request("GET", `/list/${listId}/task?${params.toString()}`);
    const tasks = data.tasks || [];
    sortByPriorityAndDate(tasks);
    log("info", `Found ${tasks.length} task(s) with status "${status}" in list ${listId}`);
    return tasks;
}
/**
 ...
 */
async function getParentSubtasksByStatus(status) {
    const parent = await request("GET", `/task/${CLICKUP_PARENT_TASK_ID}?include_subtasks=true`);
    const subtasks = parent.subtasks || [];
    const matching = subtasks.filter((s) => s.status?.status?.toLowerCase() === status.toLowerCase());
    ...
    log("info", `Found ${fullTasks.length} subtask(s) with status "${status}" under parent task ${CLICKUP_PARENT_TASK_ID}`);
    return fullTasks;
}
/**
 ...
 */
export async function addTaskCommentForUser(taskId, commentText, assigneeId) {
 ...
 */
export async function notifyTaskCreator(taskId, creator, commentText) {
    if (creator?.id) {
...
export async function getTaskComments(taskId) {
    const data = await request("GET", `/task/${taskId}/comment`);
 ...
 */
export async function findPRUrlInComments(taskId) {
...
export async function createTask(name, description) {
    ...
    return request("POST", `/list/${listId}/task`, body);
}
/**
 ...
 */
export function formatTaskForClaude(task, comments) {
    ...
    if (injectionMatches.length > 0) {
        log("warn", `Potential prompt injection detected in task ${task.id}: ${injectionMatches.join(", ")}`);
    }
    const parts = [];
 ...
 */
export async function getNewReviewFeedback(taskId) {
    const comments = await getTaskComments(taskId);
...
export async function validateStatuses() {
    ...
    if (missing.length > 0) {
        log("warn", `The following configured statuses are not in the ClickUp list: ${missing.join(", ")}`);
        log("warn", `Available statuses: ${list.statuses.map((s) => `"${s.status}"`).join(", ")}`);
        log("warn", `Please create these statuses in ClickUp or update your .env config.`);
        return false;
    }
