...
if (existsSync(configPath)) {
    try {
        ...
        console.warn(`Warning: Failed to load ${configPath}: ${err.message}`);
    }
}
export { userConfig };
// --- Required env vars ---
...
// ClickUp
export const CLICKUP_API_TOKEN = required("CLICKUP_API_TOKEN");
export const CLICKUP_LIST_ID = process.env.CLICKUP_LIST_ID || "";
export const CLICKUP_PARENT_TASK_ID = process.env.CLICKUP_PARENT_TASK_ID || "";
if (!CLICKUP_LIST_ID && !CLICKUP_PARENT_TASK_ID) {
    console.error("ERROR: Either CLICKUP_LIST_ID or CLICKUP_PARENT_TASK_ID must be set.");
    console.error("Set one in .clawup.env (in your project root) or export it.");
    process.exit(1);
}
// GitHub
...
export const RELAUNCH_INTERVAL_MS = parseInt(process.env.RELAUNCH_INTERVAL_MS || "600000", 10);
// Claude Code
export const CLAUDE_COMMAND = process.env.CLAUDE_COMMAND || "claude";
export const CLAUDE_TIMEOUT_MS = parseInt(process.env.CLAUDE_TIMEOUT_MS || "600000", 10);
export const CLAUDE_MAX_TURNS = parseInt(process.env.CLAUDE_MAX_TURNS || "50", 10);
// Branch naming
export const BRANCH_PREFIX = process.env.BRANCH_PREFIX || "clickup";
// Logging
