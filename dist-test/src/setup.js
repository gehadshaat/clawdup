...
`);
    ...
    if (existsSync(envPath)) {
        const overwrite = await ask(".clawup.env already exists. Overwrite? (y/N)", "N");
    ...
    if (useParentTask) {
        ...
        try {
            const res = await fetch(`https://api.clickup.com/api/v2/task/${parentTaskId}?include_subtasks=true`, { headers: { Authorization: apiToken } });
            if (!res.ok) {
            ...
            if (task.list?.id) {
                ...
                const listRes = await fetch(`https://api.clickup.com/api/v2/list/${listId}`, { headers: { Authorization: apiToken } });
                if (listRes.ok) {
                    const list = (await listRes.json());
                    console.log(`  List: "${list.name}" (statuses: ${list.statuses.map((s) => s.status).join(", ")})`);
                    ...
                    if (missing.length > 0) {
                        ...
                        console.log('  Run "clawup --statuses" to see the full recommended setup.');
                    }
                    else {
        ...
        console.log("\nValidating ClickUp connection...");
        try {
            const res = await fetch(`https://api.clickup.com/api/v2/list/${listId}`, { headers: { Authorization: apiToken } });
            if (!res.ok) {
                const text = await res.text();
                throw new Error(`API error ${res.status}: ${text}`);
            }
            ...
            console.log(`  Current statuses: ${list.statuses.map((s) => s.status).join(", ")}`);
            const existing = list.statuses.map((s) => s.status.toLowerCase());
            const recommended = [
                "to do",
                "in progress",
